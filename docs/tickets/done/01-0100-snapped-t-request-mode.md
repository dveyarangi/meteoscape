...
- **Depends on:** [003b — Capability carries its domain](./done/01-0060-capability-domain.md) (reach
  on the `Capability`), [m3](./done/01-0080-provider-parity-checks.md) (parity harness guards the
...
align) is designed and unbuilt; [006](./01-0130-retentive-store-freshness.md)'s store `quantize` depends on sibling
...
- **Resolution is one verb, not a mode branch (settled 2026-07-26).** A request resolves against a
  ...
  bounds(request)`). A leaf therefore **declares** a snappable T — its rolling window at its native
  series step — and calls `ground` twice: against that declaration for fetch bounds, against the
  ...
  - **`ground` joins the universal domain surface** (an ADR-0002 amendment, previously set-algebra
    only): callers hold a base `Domain` and must not dispatch to learn whether resolution is needed.
    Its argument is a `Domain` and its result an `EnumerableDomain`, so the same call serves a
    footprint, a delivered Coverage, and — at 006 — a store lattice.
...
- **Non-duplication constraints (clarified 2026-07-25)** — what keeps the X/Y instantiation a
  ...
  1. *One snap operation, in one place; still no stage 0* (revised 2026-07-26 — the align said "no
     new snap arithmetic at all"). The arithmetic does exist: it is `SnappedAxis.clip`, and it is the
     **only** site with any — leaves, Arbiter, and admission have none, and admission stays interval
     intersection on `.extent`. Because `clip` reads `datetime` / `timedelta`, m4 also narrows on
     coordinate kind **once**, which the align expected to avoid entirely. So
     [#23](../concerns.md#23-spatial-vs-temporal-regularaxis-types)'s spatial/temporal split is
     *touched*: enabling snapped X/Y brings either a spatial `clip` or that split. It stays deferred
     for the reason that deferred it — as public sibling types it would double every request-facing
     axis kind, an embedding-vocabulary cost to settle first (kept internal via construction
     autodetection, or absorbed by facade builders — recorded at #23 /
     ...
     one site away, since `clip` puts lattice arithmetic in `domain.py` beside the sampler's own.
...
- [ADR-0002](../adr/0002-data-model.md) — ~~clarify snapped resolution keeps
  ...
  **Done at the align (2026-07-25):** the mode definition inverted to bounds-only (grid supplies
  ...
  beside the existing `IntervalAxis` → `VantageAxis` one, **`ground` in the universal domain
  surface** (the base was set-algebra only — the one recorded-decision amendment this ticket
  carries), and **`Snappable`** as the type form of *"only a regular axis can be snapped-to"*.
...
- [006](./01-0130-retentive-store-freshness.md) — its open store-lattice question closes on `Snappable`
  (*narrow what `quantize` requires*: anchor and step, never enumeration), and `quantize` is restated
  as `ground`'s store-side sibling. **At landing.**
- [#21](../concerns.md#21-serves-extent-vs-project-crop-ability) narrows to the off-phase case alone
  ...
  [#23](../concerns.md#23-spatial-vs-temporal-regularaxis-types) records the one coordinate-kind
  narrowing this ticket adds and what the spatial sibling therefore costs. **At landing.**
- [glossary](../glossary.md) — *Ground* (disambiguated from the vertical datum) and *Snappable axis*.
  **At landing.**
...
- [ ] No `valid_time` splicing: one winner serves the whole (possibly shorter) window,
      single-origin.
- [ ] **The leaf carries no snapped-mode code** — no mode branch, no lattice arithmetic, no
...
- [ ] **Non-duplication guard:** the design document can truthfully state — *"enabling snapped
      X/Y is: mint the spatial sibling type with its `clip` (or take #23's split), declare a
      snappable X/Y geometry, and lift the Timeline-only invariant; zero new methods in providers,
      Arbiter, or resolution."* If that sentence is false, the shape is wrong and the align is not
      done.
