...
- **Config & secrets** — a single **typed config**, pure **data**: a set of **`SourceDef`s** — each a **`SourceKey`** (`provider` + `dataset`) plus `{impl, secret_ref, priority}` — declaring the enabled producers and their `priority`, the per-parameter `reconciler`, and the **derivation registry**. A `dataset` (offering) is fixed **at construction**, so distinct offerings are distinct configured producers keyed by `SourceKey` (v1: one per provider). The **parameter table** — the injected `ParameterDef` lookup (a swappable interface; static in v1) — is wired in the same way. Secrets are **injected at construction**, never read from globals. (Concrete config library → [`v1-requirements.md`](./v1-requirements.md).)
- **Registry** — the **producer factory**: its static catalog is **`impl-id → Provider class`**; handed the `SourceDef`s it instantiates **one configured producer per `SourceDef`** (secrets injected), keyed by **`SourceKey`** — a provider class may back several dataset-keyed instances. Its **read-only, `SourceKey`-keyed surface** (producers + each one's `priority`) is what the Weaver consumes. No wiring, no `Store`s.
- **Weaver** — the **build-time graph constructor**: from the **read-only source registry** (producers keyed by `SourceKey`, each carrying `priority`) + the **derivation registry** + store/grid config it wires the entire static DAG and **allocates every `Store`**, then steps out — the only omniscient component, absent from the request path (runtime is pure `project` recursion). Ordering flows through the registry surface, **not** a detached provider-id list. Memoized Calculator wiring and the candidate-ordering algorithm → [ADR-0004](./adr/0004-producer-resolution-and-capability.md); the precise build-time input shape → [concern #21](./concerns.md#21-weaver-build-time-input-shape).
- **Composition root** — `server.py` is **thin entrypoint**: it reads `config`, has the Registry instantiate configured producers, hands the **read-only registry + policy** to the **Weaver**, takes back the best-view Manifold, builds the Gateway, and binds surface adapters. **No ordering or construction logic of its own.**
...
- **Typed config** — `SourceDefs (SourceKey + impl / secret_ref / priority) + Arbiter config + derivation registry`; injected at construction.
...
- **[20. Provider multi-resolution offerings](./concerns.md#20-provider-multi-resolution-offerings-offering-aware-selection)** · **[21. Weaver build-time input shape](./concerns.md#21-weaver-build-time-input-shape)**.
