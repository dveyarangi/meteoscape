...
>   [011](./done/01-0120-twc-provider.md)
...
*profiles*: `server.py` declares the MCP product's own. `Settings` supplies only generic residue —
secret values, store/retention knobs, per-offering `settings` overrides. `OfferingDef.secret_ref`
...
underscores). `Settings` collects the unconsumed `METEOSCAPE_*` vars once at construction (the
raw operator vars); they travel into `compose` whole and are sliced per-impl **at the binder**
(derive-don't-parse: prefixes come from the known impl set, so no suffix grammar exists to
mis-parse). The **secret** sits at the derived slot suffix — `SecretSlot` names go impl-local (TWC
declares `api_key`; spelling `METEOSCAPE_TWC_API_KEY`, unchanged from today), one slot per impl
being `ProviderManifest.secret`'s standing contract — while every **other var under the prefix**
becomes a `settings` override, its value parsed as a JSON scalar so
`METEOSCAPE_TWC_CADENCE_HOURS=3` arrives as an int. Boot-checks refuse an impl prefix that collides
with a core `Settings` field or nests inside another impl's prefix; `build` rejects unknown
settings keys (typo guard — TWC's gains this). `open_meteo_enabled` is **dropped with no
...
- [ ] Unit + integration tests cover the keyless public boot, key-present composition of a
