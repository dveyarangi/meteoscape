...
Profile-root uses the same `StoreSpec` shape (`ProfileConfig` / `ProfileDef`) — a separate *instance*, never the same singleton as a Source store. `OfferingSpec.default_lattice` (a prebuilt `EnumerableDomain`) is excluded; declared grids are built by the factory.
...
- **Two symmetrical binders produce weave inputs.**
  ...
  source lattice; needs secrets/clock), keyed by **`SourceKey (provider, dataset)`**.
  `CalculatorBinder(CalculatorCatalog).build(CalculatorDef…)` → `CalculatorRegistry`
  (`RegisteredCalculator`: resolved manifest + outputs + inputs + priority + `stored?`), keyed by
  **`CalculatorKey (method, name)`** — the calculator peer of `SourceKey` (`method` = `fn_id`,
  `name` = the configured variant, binder-defaulted when omitted). **Both registries are keyed by their
