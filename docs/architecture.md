...
- **`snapped` resolves against the resolver's own private lattice, never at the Arbiter** — the Arbiter owns no substrate. Whichever node resolves supplies the lattice (anchor **and** step): a storing `Reservoir` `quantize`s onto its `Store` lattice, a leaf resolves onto its private vendor lattice — the store case is one instance, not the definition ([m4](./tickets/01-0100-snapped-t-request-mode.md)); the lattice surfaces only in the answer's `Coverage.domain`. Grid-alignment is **per node and per axis** → [ADR-0002](./adr/0002-data-model.md).
...
- **T** — Enumerable is the v1 path; **Snapped-T** is the window-fitting mode — a live rolling window whose servable edge the caller cannot know, and equally an archive's retention edge later → [m4](./tickets/01-0100-snapped-t-request-mode.md), [003c](./tickets/01-0110-request-shaping.md).
- **X/Y** — Enumerable point (count-1) in v1; **Snapped X/Y is reserved** for Grid realization ("this bbox at your grid registration") — the same pattern, deliberately not enabled while the Timeline-only invariant holds ([v1-requirements](./v1-requirements.md)).
- **Z** — has its own request modes (vantage / cell-addressing, [ADR-0002](./adr/0002-data-model.md)); **snapped Z is structurally inapplicable** — native levels are irregular, and only a regular axis can be snapped-to. The future "native levels within a span" want is bounded-`ANY`, deliberately unreserved.
...
A vendor-specific **leaf** Manifold that **contributes native, normalized `Coverage`s into the graph**: adapter (auth / HTTP / endpoints) + its **Normalizer** + capability/cadence/grid declarations. No storage, no children, stateless. It **authors the Coverage's provenance** at fetch — a single-fetch `Uniform` plane, stamping the run `issue_time` and deriving `expiration` from its **cadence** (`CadenceDef`, [ADR-0003](./adr/0003-provenance-and-origin.md)) — and passes that `Provenance` into the Normalizer. The Normalizer maps vendor shape → canonical **semantics** in **native** geometry — *the **Normalizer** never emits on the request Domain; read-back **kernels** are the Reservoir's ([#5](./concerns.md#5-read-time-homogenization-fidelity)). The leaf itself does answer on the geometry it was asked, at **aligned-crop** strength — see [Reservoir](#reservoir): "homogenization is not leaf-only"* — emitting **native records** grouped by shared native Domain ([ADR-0006](./adr/0006-materialization-granularity-and-store-shape.md)). `project` dispatches to the matching vendor endpoint by requested `Domain`.

**Shape wrapper + vendor Probe** (m4 landing shape; the seam record is [edge/provider.md](./edge/provider.md)). The two things that vary are independent, so they are separate types: a **shape wrapper** implements `Provider` for a family sharing one *geometry* (`TimelineProvider` — a point plus an hourly series) and owns everything algebraic — both `ground` calls, `agreed_geometry`, unit verification, `decode`, Z grouping, capability construction, provenance stamping, the aligned crop, and the `ValueError → CapabilityMismatch` translation. A **`Probe`** is the injected vendor face (a `Protocol`, inheriting nothing — the `Transport` precedent): it builds one request, parses one envelope, and returns raw named series in vendor units at the ticks delivered. The Probe speaks only the algebra's **value types** (`Interval`, `RegularAxis`, `ParameterData`, `ParameterId`) and never its manifold types, so it is not a Manifold and cannot stamp provenance, assemble a Coverage, or read a `Clock`. A new producer of a known shape adds a Probe; a new *geometry* adds a wrapper. A provider declares no lattice anywhere — its Source's `Store` is shaped by the configured `StoreSpec`; a provider whose data is already **materialized** declares an `EnumerableCapability` and wires **storeless** ([ADR-0006](./adr/0006-materialization-granularity-and-store-shape.md)).
