...
- **`ProfileConfig` is operator input for one served profile.** It contains offering enablement
  declarations (`OfferingDef`s), calculator recipes (`CalculatorSpec`s), root-store binding, and Arbiter
...
- **Weaver owns graph construction only.** `Weaver(stores: StoreFactory).weave(ProfileDef)`
  ...
  ([ADR-0004](./0004-producer-resolution-and-capability.md)). Calculator construction and scoped
  Arbiters follow the same build-time binding path. Runtime nodes hold fixed children and perform no
  catalogue lookup.
...
- **`ProfileDef` carrying `CalculatorSpec`s beside a live `SourceRegistry`.** Mixes declarations with
