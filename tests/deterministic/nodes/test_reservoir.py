...
@pytest.mark.asyncio
async def test_window_extension_refetches_the_whole_holding() -> None:
    """A held window that does not cover the required one triggers a whole-Holding refill.

    The refetch replaces the Holding rather than splicing: the child answers the *full* live timeline
    the second time, and what is served afterwards is that whole window on one origin — the old
    short window is gone, not stitched to the new one.
    """
    short = _native(lon=10.0, lat=20.0, hours=2)  # [12:00, 13:00]
    full = _native(lon=10.0, lat=20.0, hours=4)  # [12:00, 15:00] — the provider's live timeline
    child = _Widening(
    ...
    reservoir = Reservoir(_timeline_store(), child, STOPPED)
    await reservoir.project(_live_ask(hours=1))  # required ⊆ held
    assert child.calls == 1
    # Required [12:00, 15:00] is not covered by held [12:00, 13:00].
    served = await reservoir.project(_live_ask(hours=3))
    assert child.calls == 2
    assert isinstance(served, CoverageRecord)
    # Whole-Holding replace: four ticks, all from the second answer — no trace of the short window.
