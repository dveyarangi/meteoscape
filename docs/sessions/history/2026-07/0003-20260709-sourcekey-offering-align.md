...
Continues session 0002. A `/sync-arch` pass surfaced that
...
- **Weaver build-time input shape** — settled in session 0004 /
  Weaver input / `ProfileDef` (former #21 → [architecture](../../../architecture.md#config-binders-weaver)).
...
- **Code lag** — contract signatures landed in session 0004;
  `build` / `weave` behaviour and the Open-Meteo leaf remain unbuilt (per session 0002).
