...
- **Plan:** [live-window edge tolerance RFC](../../rfc/done/01-0119-live-window-edge-tolerance.md) — the declared axis answers
...
- **Depends on:** [Reservoir retention pipeline](./01-0115.0040-reservoir-retention-pipeline.md)
...
- **Horizon for rolling, containment for static — not "drop the check".** Dropping it entirely serves
  nothing once a Holding falls behind `now`, and never fills an archive's wider ask. A rolling window
  is satisfied once its horizon reaches the ask's start; a static one by containment. Mere overlap is
  wrong for both: it refetches forever on a wholly-in-gap ask, and it never fills a wider archive ask.
...
- **T-only falls out, rather than being enforced.** `RollingAxis` is temporal-by-type, so no
  ...
  [007](./01-0117-off-grid-homogenization.md) stays fatal and untouched.
- **`cadence ≤ max_lead`.** A rolling Holding is refreshed only on expiry, so a longer cadence lets the
  ...
  is exactly that trap → enforced in `CadenceDef.__post_init__`.
...
- [x] **A default request against a leaf whose series starts after the declared window opens refills
      at most once**, then serves warm — pinned by counting transport calls across two requests, not
      by inspecting the gate.
- [x] A request straddling the gap serves the overlap, and the response's first tick is the vendor's
      first delivered tick.
- [x] **The declared cadence, not the Shelf, governs refetch** (defect 2): advancing the
...
- [x] A request lying wholly inside the gap produces a **clean capability answer, not a
...
- [x] A leaf whose T reach is a **static** axis keeps exact containment — pinned by a test, so the
      rolling behaviour cannot silently generalize to the archive and obs sources.
- [x] A rolling axis whose Holding has fallen **entirely behind** the ask reports **unsatisfied** — the
...
Both selected by the [live-window edge tolerance RFC](../../rfc/done/01-0119-live-window-edge-tolerance.md):
