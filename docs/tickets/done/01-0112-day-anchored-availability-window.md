...
- **Status:** Done (2026-08-07) — `CadenceDef.window_quantum` shelves the availability window
  independently of the run clock; Open-Meteo declares its probed day-quantized truth
  (`window_quantum=24h`, `max_lead=383h`); narration floors to whole days; the live parity probe
  passes at an arbitrary hour (re-armed 003c acceptance).
- **Plan:** [RFC 0010](../../rfc/done/0010-20260806-day-anchored-availability-window.md).
- **Depends on:** [003c — Request shaping](./01-0110-request-shaping.md) (done — the default
...
narration formula) is [RFC 0010](../../rfc/done/0010-20260806-day-anchored-availability-window.md)'s.

- [x] The omitted-`end` default request succeeds against the live vendor **at any hour of the
      day**; the live parity run is that check and no longer depends on when it runs (the 003c
      acceptance probe, re-armed).
- [x] Every window within the vendor's actual forward availability serves — including an explicit
      `start` earlier today; a window wholly outside it remains `capability-mismatch`.
- [x] The declared forward reach **equals** the vendor's probed availability: nothing the
...
- [x] The narrated horizon never overstates what every request can get: whole days, floored
      ("out to 15 days"); providers with an exact-days reach (the test fakes) narrate unchanged.
- [x] Provenance behavior is unchanged: per-parameter `source` and `exp` are byte-identical to
      before — availability re-anchoring must not touch run identity or freshness.
- [x] [ADR-0003](../../adr/0003-provenance-and-origin.md) records the two-clock split (run clock:
