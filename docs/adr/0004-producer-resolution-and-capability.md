...
- **The family composes bottom-up like `project`** — leaves declare, composites derive:
  ...
  - **`DerivedCapability`** — a **Calculator**: an **input→output transform** (not a set-op). It serves
    its **co-produced output group** (1..N parameters, each present in *no* input) iff *all* its fixed
    inputs are servable through the scoped resolver; `parameters` is that output group. Derived wind is
    the **N = 2** case — one node emitting `{wind_speed, wind_direction}` from `{wind_u, wind_v}`,
...
- **A Calculator is just another candidate producer.** It declares its **output group** (its
  Capability) and is a candidate for **each** of those parameters exactly like a Source — the top Arbiter
  routes any of them to the one node. Routing a derived parameter to its Calculator is still
  **selection**; the **combination happens inside the Calculator**, so an Arbiter never computes formulas.
...
- **The graph is woven once by the Weaver; runtime nodes are dumb.** A **`CalculatorRegistry`** of
  ...
  **`CalculatorCatalog`**
  ...
  common-subexpression elimination); an in-progress set guards a **parameter-dependency cycle**. At request time nodes project their **fixed** children — no
