...
- **Status:** Done (2026-08-06) — free `start`/`end` windows ride the Snapped-T mode end to end:
  the edge authors a `SelectionDomain` with raw-instant bounds, resolution serves
  `bounds ∩ the winner's live window`, the tool narrates the served parameters plus a
  relative-horizon reach, and `Settings.default_horizon` is deleted. Landed with the live parity
  run green on the full 384-tick default and the divergence judgement re-confirmed against
  shipped behaviour.
- **Plan:** [RFC 0008](../../rfc/done/0008-20260725-003c-request-shaping.md) (**re-staged 2026-08-05**
...
- **Depends on:** [m4](./01-0100-snapped-t-request-mode.md) (done), and
  [003b — Capability carries its domain](./01-0060-capability-domain.md) (which reshapes
  [003a](./01-0040-profile-reach.md); 003a depends on 002, 002b).
...
[RFC 0008](../../rfc/done/0008-20260725-003c-request-shaping.md)'s hold banner (the clamp record). The
...
[003b](./01-0060-capability-domain.md). This ticket consumes it for **narration**: the surface
...
**Reading reach at the surface.** [003b](./01-0060-capability-domain.md) puts the per-parameter
...
- [x] `parameters` selects a subset of the 6 product params; omitting it returns all six. An
...
- [x] `start` / `end` define a free hourly window from **ISO datetimes only**: naive reads as UTC,
...
- [x] Omitting `end` fills the bound from the **folded reach end, read live** (a hint —
...
- [x] Bounds outside the served range yield the **servable part** — resolution serves
...
- [x] The tool description narrates the served parameters plus the profile's reach, both off the woven
...
- [x] Unit + mocked-transport integration tests cover subset selection, the open-`end` default,
...
- [x] **Divergent winner domains are assessed at this landing** (inherited from
      [m4](./01-0100-snapped-t-request-mode.md), [RFC 0009](../../rfc/done/0009-20260725-m4-snapped-t-request-mode.md)
      ...
      **Re-confirmed at the 2026-08-06 landing:** the shipped edge leaves the divergence loud —
      m4's pin still fails the whole request with `runtime-failure` — and no mitigation was
      built; the short-vendor disclosure case pins the neighbouring honest path (one winner,
      shorter `valid_time`) so the two stay distinguishable.
- [x] **The edge's request representation is settled here, not assumed.** This ticket is where
