...
- **X/Y** — Enumerable point (count-1) in v1; **Snapped X/Y is reserved** for Grid realization ("this bbox at your grid registration") — the same pattern, deliberately not enabled while the Timeline-only invariant holds ([v1-requirements](./v1-requirements.md)). Nothing in the algebra forbids it: what leaves it unserved is that the shipped `SnappedAxis` is **temporal by type**, so its bounds do not address a spatial axis at all. Enabling it is a spatial sibling type plus edge wiring, and a float phase-tolerance decision ([#23](./concerns.md#23-spatial-vs-temporal-regularaxis-types)).
