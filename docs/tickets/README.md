...
| Station observations | Planned | No observation source exists. The algebra can *declare* one — a station scatter reach plus declared provenance naming who/how/which-station ahead of any request ([scatter substrate](./done/01-0124.0010-scatter-substrate.md), fake-tested) — but nothing serves through it until [station observation serving](./01-0124.0030-station-observation-serving.md). Composing observations beside forecasts over a shared parameter is refused by the dominance fold until [obs+forecast reach composition](./01-0137-obs-forecast-reach-composition.md). |
...
| 0114 | [One timeline algebra, two geometries](./done/01-0114-timeline-shape-generalization.md) | Maint | Done (2026-08-22) | — | `TimelineProvider`'s algebra serves more than one geometry: it is the shared base of a family, the rolling behaviour moved into `RollingTimeline`. A member answers four questions — what geometry it publishes, how a request grounds onto it, how it signs an answer, how it refreshes fetched facts — and a guard test fails if one reimplements the algebra. Open-Meteo and TWC resolve exactly as before. *Row sits where the work happens — after the scatter substrate, before the collector slices; 0122 is spent by the 2026-08-19 renumber, so the position is the nearest unambiguous free slot before its origin.* |
...
| 0124.0020 | [Collector transport and decode](./01-0124.0020-collector-transport-and-decode.md) | — | Ready | config and secrets (the URI is a secret), composition lifetime (the connection it holds) | Meteoscape can hold a read-only connection to the operator's collector database, release it with the composition, read the station registry, and decode either observation schema into canonical values — with the schema pinned by fixtures sampled from real documents. |
| 0124.0030 | [Station observations are served](./01-0124.0030-station-observation-serving.md) | — | Planned | timeline generalization, collector transport, scatter substrate | Hourly station observations served through the projection algebra as a read-only private source in a composition of its own, discoverable place by place before any request, and signed with the network, the method, and the station. |
...
| 0137 | [Observation and forecast reaches compose](./01-0137-obs-forecast-reach-composition.md) | — | Planned (own align precedes) | station observation serving (a real scatter reach to compose) | A profile composing station observations and forecast producers over shared parameters builds and serves: a shared parameter's reach composes across a past-facing scatter and a forward-facing footprint instead of refusing the whole composition as incomparable. |
...
| 008 | [config and graceful degrade](./done/01-0123-config-secrets-degrade.md) |
