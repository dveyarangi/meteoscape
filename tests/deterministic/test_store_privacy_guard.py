"""Guard: private store Holding types stay inside `nodes/store.py`.

ADR-0006 makes the Holding table and its lattices store-private — consumed by `quantize`, the
holdings query, and read-back; never a public node face. The guard is static (same posture as
`test_probe_seam_guard`): an import of `MemoryStore` is fine; naming `_Holding` / `_HoldingKey` outside
...
_PRIVATE = frozenset({"_Holding", "_HoldingKey"})
...
def test_no_module_outside_store_references_private_holding_types() -> None:
    for path in _python_modules():
        ...
        for node in ast.walk(ast.parse(path.read_text(encoding="utf-8"))):
            if isinstance(node, ast.Name):
                ...
                # `store._Holding` reaches the private through the module object — same leak.
                used.add(node.attr)
